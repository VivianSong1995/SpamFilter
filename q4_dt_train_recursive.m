...
feat_selected = q4_split(X, Y, feat_idx);
if (size(X,1) <= c || isempty(feat_idx) || feat_selected == 0 || q4_entropy(Y)==0)
    % INSERT YOUR CODE HERE
    % you should call q4_leaf_info here
    % set the values for tree(slot,1:3)
    [label, posterior] = q4_leaf_info(Y);
    tree(slot, :) = [0, label, posterior];
    ...
    [tree, next_slot] = q4_dt_train_recursive(X(left_examples,:), Y(left_examples), feat_idx, tree, slot+1, c);
    tree(slot, 3) = next_slot;

    % recursively train right sub-tree
    [tree, next_slot] = q4_dt_train_recursive(X(right_examples, :), Y(right_examples), feat_idx, tree, next_slot, c);
end
