...
for i = 1: size(X,1)
    ...
    for j = 1:size(X,2)
        if (X(i,j) == 1)
            prob_y(1,1) = prob_y(1,1) + log(phi_y0(j,1));
            prob_y(1,2) = prob_y(1,2) + log(phi_y1(j,1));
        else
            prob_y(1,1) = prob_y(1,1) + log(1-phi_y0(j,1));
            prob_y(1,2) = prob_y(1,2) + log(1-phi_y1(j,1));
        end
    end
    prob_y(1,1) = prob_y(1,1) + log(1-phi_prior);
    prob_y(1,2) = prob_y(1,2) + log(phi_prior);
%    prob_y
    
    if (prob_y(1,1) > prob_y(1,2))
        pred_Y(i,1) = 0;
    else
        pred_Y(i,1) = 1;
    end
